...
app.post('/api/shorturl/new',  (req, res) => {
   ...
   */
   ...
   } else if ((new URL(postedURL).protocol != "http:") && (new URL(postedURL).protocol != "https:")) {
          res.json({
              error: "invalid url"
          });    
      }
        ...
        if (err) return console.error(err);
          ...
          UrlData.find({urlLong: postedURL}, (err,returnedData) => {
            ...
            if (returnedData.length) {
              ...
              var newEntry = new UrlData({urlLong: postedURL, urlShort: docNum});
